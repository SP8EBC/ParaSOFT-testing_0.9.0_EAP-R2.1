...
void SensorReset(int addr) {
	...
	while(i2cTXing != 0 && i2cRXing !=0);		// je¿eli magistala i2c nie jest zajêta, tj nie nadajê i nie odbiera
	i2cSendData(0xEC, txbuf, 0);				// wys³anie danych pod adres 0xEC czyli do czujnika
	while(i2cTXing != 0);						// czekanie na zakoñczenie transmisji
	Delay10ms();								// oczekiwanie 10ms, tyle miej wiêcej trwa resetowanie czujnika
...
int SensorReadCalData(int addr, int* cal_data) {
	...
	j = 0;
	for (i=0; i<=0xE; i+=2) {
		while(i2cTXing != 0 && i2cRXing !=0);	// je¿eli magistrala nie jest zajêta
		txbuf[0] = 0xA0 + i;					// 0xA0 to adres pierwszej sta³ej kalibracyjnej, ka¿da z nich ma 16 bitów
		txbuf[1] = '\0'; 
		i2cSendData(0xEC, txbuf, 0);			// wysy³anie adresu do odczytania
		while(i2cTXing != 0);					// oczekiwanie na zakoñczenie transmisji
		i2cReceiveData(0xED, rxbuf, 2);			// odbiór danych z czujnika
		while(i2cRXing != 0);
		*(cal_data + j) = ((i2cRXData[0] << 8) | i2cRXData[1]);		// przepisywanie danych z buforów do tablicy
		j++;
	}
	if (crc4(cal_data) == 0x0B)					// sprawdzanie poprawnoœci odebranych danych
		return 0;
	else
		return -1;
}
...
long int SensorStartMeas(int param_to_meas) {
	int txbuf[] = { 0x00, 0x00};
	long int output;
	if(param_to_meas == 0x01) {					// pomiar D1
		txbuf[0] = 0x44;						// oversampling 1024
		i2cSendData(0xEC,txbuf, 0);				// wys³anie rozkazu rozpoczêcia pomiaru
		while (i2cTXing != 0);
		Delay10ms();							// oczekiwanie na zakoñczenie pomiaru
		txbuf[0] = 0x00;
	...
	if(param_to_meas == 0x02) {					// pomiar D2
...
// funkcja obliczaj¹ca sumê kontroln¹ CRC4 z wspó³czynników kalibracyjncyh
